# Hands-on Docker Workshop ENEI 2016
This repository holds the code related with the Hands-on Docker Workshop in ENEI@UA 2016

The following steps were taken from the official Self-paced [Docker Tutorials and Labs](https://github.com/docker/labs) and the official Self-paced [Docker Training](https://training.docker.com).
...
7. [Final exercise](#finalexercise)
...
First things first! You will follow this [Setup](https://github.com/docker/labs/blob/master/beginner/chapters/setup.md#setup) to install Docker in your machine.
...
| USER        | Sets the user name or UID to use when running the image (including execution _instructions_ from the same Dockerfile). |
...
| ONBUILD     | Adds a trigger _instruction_ to the image to be executed at a later time, when the image is used as the base for another build. |
...
```docker
FROM ubuntu:14.04
MAINTAINER Tiago Pires <tiago.pires@mindera.com>
...
## <a name='finalexercise'></a> Final Exercise
So now that your Docker knowledge is up to date, you can challenge your self by doing a final exercise.

You'll be guided through the creation and customization of a voting app. Click [here](https://github.com/docker/labs/blob/master/beginner/chapters/votingapp.md#30-run-a-multi-container-app-with-docker-compose) to continue.
